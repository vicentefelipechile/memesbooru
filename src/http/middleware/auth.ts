...
async function getAuthUser(c: Context<{ Bindings: Env }>): Promise<AuthUser | null> {
	...
	const row = await db
		...
		.bind(hash)
		.first<{ user_id: number; expires_at: number; revoked_at: number | null; username: string; rank: string; status: string }>();
	if (!row || row.revoked_at || row.expires_at < Date.now()) return null;
	return {
		id: row.user_id,
		username: row.username,
		rank: row.rank as AuthUser['rank'],
...
export async function requireAuth(c: Context, next: Next) {
	const user = await getAuthUser(c as Context<{ Bindings: Env }>);
