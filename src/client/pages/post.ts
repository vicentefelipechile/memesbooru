  ...
  return `
    ...
    <main class="page post-page">
      <div class="post-media">
        ${restricted ? `<p class="restricted">Video solo para usuarios trusted. Gana confianza publicando y participando.</p>` : `
...
export function bindPost(publicId: string): void {
  ...
  q("#vote-up")?.addEventListener("click", async () => { await fetch(`/api/post/${publicId}/rating`, { method: "POST", headers: { "content-type": "application/json" }, credentials: "include", body: JSON.stringify({ value: 1 }) }); location.reload(); });
  q("#vote-down")?.addEventListener("click", async () => { await fetch(`/api/post/${publicId}/rating`, { method: "POST", headers: { "content-type": "application/json" }, credentials: "include", body: JSON.stringify({ value: -1 }) }); location.reload(); });
  q("#fav-btn")?.addEventListener("click", async () => { await fetch(`/api/post/${publicId}/favorite`, { method: "POST", credentials: "include" }); location.reload(); });
  q("#report-btn")?.addEventListener("click", async () => { const reason = prompt("Motivo del reporte:"); if (!reason) return; await fetch("/api/moderation/reports", { method: "POST", headers: { "content-type": "application/json" }, credentials: "include", body: JSON.stringify({ targetType: "post", targetId: 0, reason }) }); alert("Reportado"); });
  // Cargar comentarios
  fetch(`/api/comments/post/${publicId}`).then((r) => r.json()).then((j: unknown) => { const data = j as { data: { id: number; body: string; author_id: number }[] };
    const el = q("#comments");
    if (!el) return;
    ...
    q("#comment-form")?.addEventListener("submit", async (e) => { e.preventDefault(); const body = (q("#c-body") as HTMLTextAreaElement).value; await fetch(`/api/comments/post/${publicId}`, { method: "POST", headers: { "content-type": "application/json" }, credentials: "include", body: JSON.stringify({ body }) }); location.reload(); });
  });
}
