    ...
    <div class="post-layout">
      <section class="post-main">
        <div class="post-media">
          ${
						restricted
							? `<p class="restricted">Este video es solo para usuarios trusted. Gana confianza publicando y participando.</p>`
							: `<img src="/api/posts/${publicId}/variants/medium" alt="post ${publicId}" loading="eager" />`
					}
        </div>
	...
	form.addEventListener('submit', async (e) => {
		...
		if (list) {
			list.insertAdjacentHTML(
				'beforeend',
				`<article class="comment"><div class="byline"><span class="author">${escapeHtml(author)}</span> · ${formatDate(Date.now())}</div><div class="body">${escapeHtml(body)}</div></article>`,
			);
