...
async function processMedia(env: Cloudflare.Env & { DB: D1Database; MEDIA_BUCKET: R2Bucket; QUARANTINE_BUCKET: R2Bucket }, postId: number): Promise<void> {
	...
	if (existing) {
		...
		await db
			...
			.bind(`media/${postId}/low.avif`, Date.now(), postId)
			.run();
		return;
	}
	...
	const lowKey = `media/${
		asset.checksum
			? Array.from(new Uint8Array(asset.checksum.slice(0, 4)))
	...
	await db.batch([
		db
			...
			.bind(asset.id, 'low', lowKey, 'image/avif', dummy.length, 'low', 'public', now),
		db
			.prepare('INSERT INTO media_variants (media_asset_id, variant_name, object_key, mime_type, byte_size, quality_class, visibility, created_at) VALUES (?,?,?,?,?,?,?,?)')
			.bind(asset.id, 'medium', medKey, 'image/avif', dummy.length, 'medium', 'public', now),
