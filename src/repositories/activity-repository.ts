...
export async function getActivity(db: DB, userId: number): Promise<{ approved_posts: number } | null> {
	return queryOne(db, 'SELECT approved_posts FROM user_activity WHERE user_id = ?', [userId]);
}

export async function getUserRankMeta(db: DB, userId: number): Promise<{ rank: string; created_at: number } | null> {
	return queryOne(db, 'SELECT rank, created_at FROM users WHERE id = ?', [userId]);
}
