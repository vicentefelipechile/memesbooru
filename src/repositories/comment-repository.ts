...
export class CommentRepository {
	...
	async listByPost(postId: number, cursor?: string, limit = 20): Promise<CommentResult[]> {
		...
		if (cursorVal) {
			return queryAll<CommentResult>(this.db, `${base} AND (c.created_at > ? OR (c.created_at = ? AND c.id > ?)) ORDER BY c.created_at ASC, c.id ASC LIMIT ?`, [postId, cursorVal.created_at, cursorVal.created_at, cursorVal.id, limit]);
		}
	...
	buildInsertCommentStatement(row: InsertCommentStatementData): D1PreparedStatement {
		return this.db
	...
	async create(data: CreateCommentData): Promise<CommentRow['id']> {
		...
		await batch(this.db, [
			this.buildInsertCommentStatement({
				id: nextId,
				postId: data.postId,
			...
			this.db.prepare('UPDATE posts SET comment_count = comment_count + 1, updated_at = ? WHERE id = ?').bind(now, data.postId),
			this.db.prepare('UPDATE post_listing SET comment_count = comment_count + 1 WHERE post_id = ?').bind(data.postId),
			this.db.prepare('UPDATE user_activity SET comments_count = comments_count + 1, last_comment_at = ?, updated_at = ? WHERE user_id = ?').bind(now, now, data.authorId),
		]);
